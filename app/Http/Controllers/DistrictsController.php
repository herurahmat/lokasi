        ...
        try {
            ...
            if (!$validatedData) {
                return response()->json([
                    'status' => 300,
                    'data' => array(),
                    'message' => 'Validation Error. Require regency_id'
                ]);
            }
            ...
            $query = Districts::where('districts.id', '!=', '')->select('districts.id as district_id', 'districts.name as district_name','regencies.id as regency_id', 'regencies.name as regency_name', 'provinces.id as province_id', 'provinces.name as province_name')
